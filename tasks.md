...
- [ ] **Re-Export Missing Core API Surface in `src/index.ts**`
- **Status**: ⏳ Todo
- **Target**: 2026-08-11
...
- [ ] Export `checkHeadingOrder` and `checkHeadingOrderReport` from `./utils/check-heading-order-report`.
- [ ] Verify entrypoint exports via local build check.

- [ ] **Fix Testing Subpath Type Exports in `src/**testing**/index.ts**`
- **Status**: ⏳ Todo
- **Target**: 2026-08-11
...
- [ ] Re-export all type definitions from `./types.d`.
- [ ] Run `pnpm run build` and confirm `dist/testing.d.mts` contains full matcher interfaces.
