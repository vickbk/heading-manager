...
describe("handleReadmeCliError", () => {
  ...
  it("delegates error unwrapping to unwrapReadmeErrorMessages and prepends HEADER_TEXT", () => {
    const unwrapSpy = vi
  ...
  it("returns a single unwrapped error message under HEADER_TEXT without dividers", () => {
  ...
  it("joins multiple unwrapped error messages using a 50-character dashed separator block under HEADER_TEXT", () => {
    vi.spyOn(unwrapModule, "unwrapReadmeErrorMessages").mockReturnValue([
  ...
  it("handles empty error arrays by returning an empty string without printing HEADER_TEXT", () => {
    vi.spyOn(unwrapModule, "unwrapReadmeErrorMessages").mockReturnValue([]);
  ...
  describe("Header & Formatting Resilience", () => {
    it("starts with HEADER_TEXT whenever non-empty error messages are returned", () => {
      vi.spyOn(unwrapModule, "unwrapReadmeErrorMessages").mockReturnValue([
    ...
    it("preserves exact inner formatting and multiline content around separators after HEADER_TEXT", () => {
      vi.spyOn(unwrapModule, "unwrapReadmeErrorMessages").mockReturnValue([
        "Block A Header\n  - Detail A1\n  - Detail A2",
        "Block B Header\n  - Detail B1",
      ]);
    ...
    it("handles non-Error objects passed as top-level arguments under HEADER_TEXT", () => {
      vi.spyOn(unwrapModule, "unwrapReadmeErrorMessages").mockReturnValue([
