...
describe("normalizeHeadingText", () => {
  ...
  it("removes markdown emphasis, links, and inline code syntax", () => {
    ...
    expect(normalizeHeadingText("`TypeScript Support` ")).toBe("typescript support");
  });
  ...
  it("strips punctuation and repeated whitespace without affecting section identity", () => {
    ...
    expect(normalizeHeadingText("  Architecture   &   Module Isolation Policy  ")).toBe(
      "architecture module isolation policy",
    );
  });

  it("handles badge markdown and HTML tags safely", () => {
    expect(normalizeHeadingText("![npm version](https://example.com/badge.svg)")).toBe("");
    expect(normalizeHeadingText("<span>Accessibility</span>")).toBe("accessibility");
  });
});
